...
import type { ExecutionRepository, SharedWorkflowRepository, WorkflowRepository } from '@n8n/db';
import { mock } from 'jest-mock-extended';
import type { BinaryDataService, Cipher, ErrorReporter } from 'n8n-core';
...
import type { ChatHubTitleService } from '../chat-hub-title.service';
...
describe('ChatHubService', () => {
	...
	beforeEach(() => {
		...
		service = new ChatHubService(
			...
			mock<ChatHubSessionRepository>(),
			mock<ChatHubMessageRepository>(),
			chatHubAgentService,
			mock<ChatHubModelsService>(),
			chatHubAttachmentService,
			mock<ChatStreamService>(),
			mock<ChatHubExecutionService>(),
			mock<ChatHubTitleService>(),
			mock<ChatHubWorkflowService>(),
			mock<GlobalConfig>(),
		);
	...
	describe('buildConversationHistory', () => {
		describe('message history handling', () => {
			it('should handle empty history', async () => {
				expect(
					service.buildConversationHistory([], { provider: 'openai', model: 'gpt-4-turbo' }, []),
				).toHaveLength(0);
			});
			...
			it('should handle multiple messages', async () => {
				...
				const messageValues = await service.buildConversationHistory(
					mockHistory,
