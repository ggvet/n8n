...
> {
	...
	create(config: BuilderSubgraphConfig) {
		...
		const includeScriptExecution = config.featureFlags?.scriptExecution === true;
		...
		if (includeScriptExecution) {
			// Script execution mode: single tool that handles everything
			...
			// Standard mode: individual tools for structure + configuration
			const baseTools = [
				// Structure tools
		...
		const basePrompt = buildBuilderPrompt({ includeExamples });
		const systemPromptText = includeScriptExecution
			? `${basePrompt}\n\n${SCRIPT_EXECUTION_CONDENSED}\n\n${CONFIGURATOR_SCRIPT_TOOLS}\n\n${SCRIPT_TYPE_DEFINITIONS}`
			: basePrompt;
